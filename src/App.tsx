...
function App() {
  ...
  const updateSensorData = useCallback(
    (data: SensorData) => {
      ...
      if (data.temperature !== 0 || data.soilMoisture !== 0) {
         // saveSensorReading(data); // <-- Commented out to prevent Supabase 401 error
      }
    },
    [thresholds]
  );
      ...
      const unsubscribe = onValue(
        ...
        (snapshot) => {
          ...
          if (data && typeof data === 'object') {
            ...
            // Get the last key (which is the latest reading)
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // const readings = await getLatestReadings(24); // <-- Commented out
        // setHistoricalReadings(readings as Reading[]); // <-- Commented out

        // const thresholdData = await getThresholds('public'); // <-- Commented out
        // setThresholds(thresholdData); // <-- Commented out
      } catch (err) {
        console.error('Error loading initial data:', err);
      }
    };

    // loadData(); // <-- Commented out to prevent Supabase 401/400 errors
  }, []); // Runs once on mount
